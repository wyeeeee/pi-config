...
async function runSingleAgent(
	...
	// Prefer the parent session's current model (passed as overrideModel) so the
	// subagent tracks the model actually in use; fall back to the agent's model.
...
export default function (pi: ExtensionAPI) {
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			const extAgentsDir = path.join(path.dirname(fileURLToPath(import.meta.url)), "agents");
			const discovery = discoverAgents(ctx.cwd, agentScope, extAgentsDir);
